...
def stat_plot(ylabel, res, aggregate_time_to_range):
...
if __name__ == '__main__':
    ...
    # the input data
    net_name = dc.net_name
    print("***", net_name)

    # pair_contacts = dc.pair_contacts
    ...
    for aggregate_time_to in aggregate_time_to_range:
        ...
        dcagg = dc.aggregate_to(aggregate_time_to, makecopy=True)

        g = dcagg.get_timegraph()
        ...
        for i in range(1):
            dcagg.pair_contacts = null_model(dcagg.pair_contacts)  # This should be safe because no other properties are changed in generating the null model

            g_null = dcagg.get_timegraph()  # Also safe because the timegraph is not cached
        ...
        print("  -->", dc.net_suffix, "\t", g_stability, "\t", g_null_stability, "\t", "diff:", res)
        differential_stability.append(res)

    # plot the grid test
    xlabel = "Aggregation period (s)"
    stat_plot("Stability", differential_stability, aggregate_time_to_range)
